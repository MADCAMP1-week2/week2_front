...
const H_MIN = 35, H_WEEK = 150, H_NAVI = 65, H_FULL = H;
const CELL_H = 66, H_GAP = 2, V_GAP = 7, ROW_H = CELL_H + V_GAP * 2;
...
export default function HandleCalendarPanel({ y }) {
  ...
  const handleBarSt = useAnimatedStyle(()=>({ opacity:interpolate(progress.value,[0,0.25],[0,1],Extrapolate.CLAMP) }));
  const titleSt = useAnimatedStyle(()=>({ opacity:interpolate(progress.value,[0.45,0.3],[0,1],Extrapolate.CLAMP) }));
  ...
  const rowStyles = Array.from({length:6},(_,row)=>{
    ...
    return useAnimatedStyle(()=>({ opacity:interpolate(progress.value,[st,0],[0,1],Extrapolate.CLAMP), transform:[{translateY:interpolate(progress.value,[st,0],[20,0],Extrapolate.CLAMP)}] }));
  });
